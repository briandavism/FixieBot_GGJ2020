...
float stat_decrypt = 0;         // TEMP: Robot's decrypt, starts from 0
...
void loop() {
  ...
  if (state == IDLE) { 
    ...
    firstIdleFrame = true;
    playDecryptingDone();
    stat_decrypt = 0; // Just resetting the decrypt back to zero for now
  }
  ...
  // If decrypt is done, switch to decrypting_done. Only place where the decrypt is increased is from beingdecrypted.
  if (stat_decrypt >= 100) {
